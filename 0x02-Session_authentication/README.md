...
- `File`: [api/v1/app.py](./api/v1/app.py), [api/v1/views/users.py](./api/v1/views/users.py)
...
Update `api/v1/auth/auth.py` by adding the method d`ef session_cookie(self, request=None):` that returns a cookie value from a request:
...
Create an instance method d`ef current_user(self, request=None):` (overload) that returns a `User` instance based on a cookie value:
...
- If the Session ID of the request is not linked to any User ID, return `False` - you must use s`elf.user_id_for_session_id(...)`
...
- `GitHub repository`: `alx-backend-user-data`
- `Directory`: `0x02-Session_authentication`
- `File`: [api/v1/auth/session_auth.py](./api/v1/auth/session_auth.py), [api/v1/views/session_auth.py](./api/v1/views/session_auth.py)
